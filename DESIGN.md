...
| P2.6b | Split into four slices. **DISTINCT ON — done** (`deduplicateRowsOn` now wired into the MVCC table-scan and join paths). Remaining: `GROUPING SETS` / `CUBE` / `ROLLUP` + `GROUPING()` emit only the grand total; window frames compute wrong moving values and `MIN`/`MAX`/`NTH_VALUE`/`LAG`/`LEAD`/`NTILE` are unsupported as window functions; `MERGE` result differences. |
