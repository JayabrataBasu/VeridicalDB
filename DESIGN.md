...
| P2.6b | Split into four slices. **DISTINCT ON — done.** **GROUPING SETS / CUBE / ROLLUP + `GROUPING()` — done** (shared `computeGroupingSets` in `exec_grouping_sets.go`). Remaining: window frames compute wrong moving values and `MIN`/`MAX`/`NTH_VALUE`/`LAG`/`LEAD`/`NTILE` are unsupported as window functions; `MERGE` result differences. |
