		...
		if infoMeta, ok := informationSchemaTable(stmt.TableName); ok {
			...
			if e.cteData != nil {
				if cteResult, ok := e.cteData[stmt.TableName]; ok {
					return selectFromCTE(stmt, cteResult, e.evalExpr, e.evalCondition)
				}
			}
...
func (e *Executor) executeSelectWithGroupingSets(stmt *ast.SelectStmt, meta *catalog.TableMeta) (*Result, error) {
	...
	return computeGroupingSets(allRows, meta.Schema, stmt, e.evalCondition)
}
...
func (e *Executor) executeSelectWithWindowFunctions(stmt *ast.SelectStmt, meta *catalog.TableMeta) (*Result, error) {
	...
	for colIdx, col := range stmt.Columns {
		...
		if wf, ok := col.Expression.(*ast.WindowFuncExpr); ok {
			...
			windowValues, err := computeWindowFunction(wf, allRows, meta.Schema, func(x ast.Expression) (catalog.Value, error) {
				return e.evalExpr(x, meta.Schema, nil)
			})
			if err != nil {
				return nil, err
			}
